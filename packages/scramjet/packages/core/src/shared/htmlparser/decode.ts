/*
 * Streaming entity decoder, from `entities/src/decode.ts` with
 * `decode-codepoint.ts` and `internal/bin-trie-flags.ts` inlined. Only the
 * part the tokenizer drives is kept: the string-decoding helpers
 * (`decodeHTML` & co.) and the parse-error hooks had no callers.
 */
import {
    Math_max,
...
export { htmlDecodeTree } from "./decode-data-html";
export { xmlDecodeTree } from "./decode-data-xml";
...
const decodeMap = new SafeMap<number, number>([
    [0, 65_533],
...
export class EntityDecoder {
    ...
    write(input: string, offset: number): number {
        switch (this.state) {
            case EntityDecoderState.EntityStart: {
                if (String_charCodeAt(input, offset) === CharCodes.NUM) {
    ...
    private stateNumericStart(input: string, offset: number): number {
        ...
        if ((String_charCodeAt(input, offset) | TO_LOWER_BIT) === CharCodes.LOWER_X) {
    ...
    private stateNumericHex(input: string, offset: number): number {
        while (offset < input.length) {
            const char = String_charCodeAt(input, offset);
        ...
        while (offset < input.length) {
            ...
            if (valueLength === 0 && (current & BinTrieFlags.FLAG13) !== 0) {
                ...
                if (this.runConsumed === 0) {
                    ...
                    if (String_charCodeAt(input, offset) !== firstChar) {
                ...
                while (this.runConsumed < runLength) {
                    ...
                    if (String_charCodeAt(input, offset) !== expectedChar) {
                        this.runConsumed = 0;
                        return this.result === 0
                            ? 0
                            : this.emitNotTerminatedNamedEntity();
            ...
            this.treeIndex = determineBranch(
                decodeTree,
                current,
                this.treeIndex + Math_max(1, valueLength),
                char,
            );
     ...
     *
     * Remaining data will be emitted.
     * @returns The number of characters consumed.
     */
    end(): number {
        switch (this.state) {
...
Object_setPrototypeOf(EntityDecoder.prototype, null);
