...
import { SafeMap, type Sequence, sequence } from "./safe";
...
const Sequences = {
    Empty: sequence([]),
    Cdata: sequence([0x43, 0x44, 0x41, 0x54, 0x41, 0x5b]), // CDATA[
    CdataEnd: sequence([0x5d, 0x5d, 0x3e]), // ]]>
    CommentEnd: sequence([0x2d, 0x2d, 0x21, 0x3e]), // `--!>`
    Doctype: sequence([0x64, 0x6f, 0x63, 0x74, 0x79, 0x70, 0x65]), // `doctype`
    IframeEnd: sequence([0x3c, 0x2f, 0x69, 0x66, 0x72, 0x61, 0x6d, 0x65]), // `</iframe`
    NoembedEnd: sequence([
        0x3c, 0x2f, 0x6e, 0x6f, 0x65, 0x6d, 0x62, 0x65, 0x64,
    ]), // `</noembed`
    NoscriptEnd: sequence([
        0x3c, 0x2f, 0x6e, 0x6f, 0x73, 0x63, 0x72, 0x69, 0x70, 0x74,
    ]), // `</noscript`
    NoframesEnd: sequence([
        0x3c, 0x2f, 0x6e, 0x6f, 0x66, 0x72, 0x61, 0x6d, 0x65, 0x73,
    ]), // `</noframes`
    Plaintext: sequence([
        0x3c, 0x2f, 0x70, 0x6c, 0x61, 0x69, 0x6e, 0x74, 0x65, 0x78, 0x74,
    ]), // `</plaintext`
    ScriptEnd: sequence([0x3c, 0x2f, 0x73, 0x63, 0x72, 0x69, 0x70, 0x74]), // `</script`
    StyleEnd: sequence([0x3c, 0x2f, 0x73, 0x74, 0x79, 0x6c, 0x65]), // `</style`
    TitleEnd: sequence([0x3c, 0x2f, 0x74, 0x69, 0x74, 0x6c, 0x65]), // `</title`
    TextareaEnd: sequence([
        0x3c, 0x2f, 0x74, 0x65, 0x78, 0x74, 0x61, 0x72, 0x65, 0x61,
    ]), // `</textarea`
    XmpEnd: sequence([0x3c, 0x2f, 0x78, 0x6d, 0x70]), // `</xmp`
};
 ...
 */
const specialStartSequences = new SafeMap<number, Sequence>([
...
export default class Tokenizer {
    ...
    private currentSequence: Sequence = Sequences.Empty;
    ...
    private stateSpecialStartSequence(c: number): void {
        ...
        if (this.sequenceIndex < this.currentSequence.length) {
            ...
            if (this.sequenceIndex === 3) {
                ...
                // Scramjet: `<noscript>` is raw text when scripting is enabled.
    ...
    private fastForwardTo(c: number): boolean {
        while (++this.index < this.buffer.length + this.offset) {
            if (String_charCodeAt(this.buffer, this.index - this.offset) === c) {
                return true;
            }
        }
    ...
    private stateInEntity(): void {
        ...
        if (length >= 0) {
            ...
            if (
                ...
                String_charCodeAt(this.buffer, indexInBuffer) === CharCodes.Amp
            ) {
                this.state = this.baseState;
                this.index -= 1;
                return;
    ...
    private parse() {
        while (this.shouldContinue()) {
            const c = String_charCodeAt(this.buffer, this.index - this.offset);
            switch (this.state) {
                case State.Text: {
                    this.stateText(c);
                    break;
    ...
    private handleTrailingCommentLikeData(endIndex: number): boolean {
        ...
        if (this.currentSequence === Sequences.CdataEnd) {
            ...
            const offset = this.xmlMode
                ? 0
                : Math_min(this.sequenceIndex, Sequences.CommentEnd.length - 1);
...
Object_setPrototypeOf(Tokenizer.prototype, null);
