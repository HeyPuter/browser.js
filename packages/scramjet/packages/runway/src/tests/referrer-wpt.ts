...
const vendorRoot = path.join(__dirname, "wpt", "vendored");
...
async function collectVendoredWptPages() {
	const root = path.join(vendorRoot, "referrer-policy");
	const pages: string[] = [];
	...
	async function walk(current: string) {
		...
		for (const entry of entries) {
			...
			if (entry.name === "fetch.http.html") {
				pages.push(path.relative(vendorRoot, fullPath));
			}
		}
	}

	await walk(path.join(root, "gen"));
