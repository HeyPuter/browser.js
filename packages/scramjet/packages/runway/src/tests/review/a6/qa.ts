...
// them to disagree is a stale `$top` naming another host (stale-top below)
{
	...
	tests.push(a);
	const b = basicTest({
		name: "rv6-qa-modemismatch-control",
		js: `assert(true);`,
	});
	...
	const FILES: Record<string, [string, string]> = {
		...
		"/pagec": [
			H,
			`<!doctype html><body><script src="/pagec.js"></script><script>setTimeout(() => runTest(async () => { assertEqual(window.__pagecRan, 1, "pagec.js ran"); }), 500);</script></body>`,
		],
		"/pagec.js": [J, `window.__pagecRan = 1;`],
	};
	const c = serverTest({
